...
class BaseTech:
	...
	def _add_submod(self, name: str, inst: Instance):
		if name is None:
	...
	def add_gate(self, typ: str, name: str|None=None, **ports: dict[str, PortVal]):
		# TODO: gate golfing
	...
	def add_mux(self, inputs: list[PortVal], sel: list[PortVal], y: Signal, name=None):
		assert len(inputs) in self.avail_mux_sizes()
		assert len(sel) == len(inputs).bit_length()
		add_gate(type=f"mux{len(inputs)}", y=y, 
