...
class FPGATop(Elaboratable):
    def __init__(self, build_dir):
        ...
        self.io = Signal(38)
        self.ram0_rstn = Signal()
        self.ram1_rstn = Signal()
        self.ports = [self.io, self.ram0_rstn, self.ram1_rstn]
    ...
    def elaborate(self, platform):
        ...
        m.d.comb += self.ram0_rstn.eq(1)
        m.d.comb += self.ram1_rstn.eq(1)
        return m
...
def main():
    ...
    if args.build:
        ...
        subprocess.run(["yosys", "-ql", Path(args.build_dir) / "synth.log", "-p",
            ...
            top_verilog, spimem_verilog, vex_verilog], check=True)
