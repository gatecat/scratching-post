...
from .linux_soc import SoCWrapper
...
def do_fixup(args):
    ...
    script = "/home/gatecat/mpw4/thirdparty/open_pdk/C4M.Sky130/libs.tech/klayout/bin/conv_c4msky130_to_sky130.py"
...
def do_pnr(args):
    ...
    sys.path.append("/home/gatecat/mpw4/thirdparty/open_pdk/C4M.Sky130/libs.tech/coriolis/techno/etc/coriolis2")
    ...
    cell_name = "user_project_core_vex"
   ...
   # etesian.spaceMargin is ignored if the coreSize is directly set.
    ...
    conf.cfg.harness.path                = '/home/gatecat/mpw4/resources/user_project_wrapper.def'
    conf.useSpares           = True
    ...
    conf.bRows               = 2
    conf.chipName            = 'chip'
    conf.coreSize            = ( u( 270*10.0), u( 320*10.0) )
...
def main():
    ...
    if args.synth:
        top = Sky130Top(args.build_dir)
        output = verilog.convert(top, name="user_project_core_vex", ports=top.ports)

        top_verilog = Path(args.build_dir) / "top.v"
        spimem_verilog = Path(__file__).parent.parent / "cores" / "spimemio.v"
        vex_verilog = Path(__file__).parent.parent / "verilog_gen" / "VexRiscvLinuxAsic.v"
        abc_constr = Path(__file__).parent.parent / "soc" / "abc.constr"
        liberty = "/home/gatecat/mpw4/thirdparty/open_pdk/C4M.Sky130/libs.ref/StdCellLib/liberty/StdCellLib_slow.lib"
...
read_verilog {top_verilog} {spimem_verilog} {vex_verilog}
synth -flatten -top user_project_core_vex
...
setundef -zero
clean -purge
write_blif {args.build_dir}/user_project_core_vex.blif
write_json {args.build_dir}/user_project_core_vex.json
stat"""
