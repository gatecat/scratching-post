...
void spiflash_model::step(unsigned timestamp) {
    ...
    if ((*csn) && !s.last_csn) {
        ...
        if (s.data_width == 4)
            s.curr_byte = (s.curr_byte << 4U) | (*d_o & 0xF);
        else
            s.curr_byte = (s.curr_byte << 1U) | (*d_o & 0x1);
        ...
        if (s.data_width == 4) {
            *d_i = ((s.out_buffer >> 4U) & 0xFU);
        } else {
            *d_i = (((s.out_buffer >> 7U) & 0x1U) << 1U);
        }
    }
    s.last_clk = bool(*clk);
    s.last_csn = bool(*csn);
...
void gpio_model::step(unsigned timestamp) {
    ...
    *i = uint8_t((input_data & ~oe_value) | (o_value & oe_value));
    s.o_last = o_value;
    s.oe_last = oe_value;
}
