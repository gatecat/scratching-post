...
module user_module_341404507891040852(
  ...
  output [7:0] io_out
    ...
    sky130_fd_sc_hd__clkbuf_4 mode_clkbuf(.A(io_in[0]), .X(cfg_mode));
    sky130_fd_sc_hd__clkbuf_4 frameinc_clkbuf(.A(io_in[1]), .X(cfg_frameinc));
    sky130_fd_sc_hd__clkbuf_4 framestrb_clkbuf(.A(io_in[2]), .X(cfg_framestrb));
    sky130_fd_sc_hd__clkbuf_4 data_clkbuf(.A(io_in[3]), .X(cfg_dataclk));

    sky130_fd_sc_hd__buf_4 sel_buf[3:0] (.A(io_in[7:4]), .X(cfg_sel));
    ...
    sky130_fd_sc_hd__buf_4 din_buf[7:0] (.A(io_in), .X(fab_din));
    ...
    generate
        ...
        for (yy = 0; yy < H; yy = yy + 1'b1) begin: y_c
            for (xx = 0; xx < W; xx = xx + 1'b1) begin: x_c
                wire ti, bi, li, ri;
                if (yy > 0) assign ti = cell_q[yy-1][xx]; else assign ti = fab_din[xx + 4];
