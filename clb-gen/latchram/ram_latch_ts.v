module ram_blk #(parameter A=5, D=8, P=1)(input WSTB, input [A-1:0] WA, input [D-1:0] WD, input [A*P-1:0] RA, output [D*P-1:0] RD);
	...
	generate
		genvar ii, jj, kk;
		for (ii = 0; ii < P; ii = ii + 1'b1) begin:port
			wire [A-1:0] port_addr = RA[ii*A +: A];
			wire [D-1:0] port_rdata;
			for (jj = 0; jj < 2**A; jj = jj + 1'b1) begin:word
				wire word_seln = ~(port_addr == jj);
				for (kk = 0; kk < D; kk = kk + 1'b1) begin:bits
					sky130_fd_sc_hd__einvn_1 rfbit_i (
						.A(ram[jj][kk]),
						.TE_B(word_seln),
						.Z(port_rdata[kk])
					);
